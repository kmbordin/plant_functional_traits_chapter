...
#png("results/traits_FD_ecosystem.png", units="in", width=13, height=10, res=300)
...
fd3 <- data %>%
  ...
  mutate(ecosystem = replace(ecosystem, ecosystem == "grassland", "Grassland")) %>%
  rename(Ecosystem = ecosystem) %>% 
  group_by(Ecosystem) %>% #region aqui para calcular para as regioes tropical e temperada
...
#region X-squared = 6.6667, df = 5, p-value = 0.2466

g = fd3 %>% filter(Ecosystem == "Grassland") 
#g = fd3 %>% filter(regiao == "temperate") 
g1 = chisq.test(g$n) #X-squared = 1.3, df = 2, p-value = 0.522
#temperate: X-squared = 4.3, df = 2, p-value = 0.1165

f = fd3 %>% filter(Ecosystem == "Forest")
#f = fd3 %>% filter(regiao == "tropical") 
f1 = chisq.test(f$n) #X-squared = 5.8462, df = 2, p-value = 0.05377
#tropical: X-squared = 1.625, df = 2, p-value = 0.4437
...
fd3%>% 
  ...
  labs(x = "", y = "Frequency of papers (%)", title = "Relationhip between functional diversity and productivity") + 
...
p1 = ggplot(all, aes(x = regiao, y= frequencia , fill= FD))+
  ...
  labs(x = "", y = "Frequency (%)", title = "Functional diversity and productivity relationship \n across different regions") +  theme_minimal()  + 
  theme(legend.position = "bottom",
        #legend.position = c(0.9,0.05),
        axis.text=element_text(size=15),
        strip.text = element_text(size = 15), 
...
summarise = data %>% 
  ...
  mutate(FD = replace(FD, FD == "ns", "no relationship")) %>%
  mutate(FD = replace(FD, FD == "positive", "positive")) %>%
  mutate(prod.metric = replace(prod.metric, prod.metric == "1", "temporal")) %>%
  mutate(prod.metric = replace(prod.metric, prod.metric == "2", "stocks")) %>%
...
length(summarise$Ecosystem)
filter(summarise, Region %in% esse)
a = filter(summarise, `Productivity measurement` != "both")

esse <- c("tropical", "temperate")
filter(a, Region %in% esse)
